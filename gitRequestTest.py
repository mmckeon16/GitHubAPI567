...
class gitRequestTest(unittest.TestCase):
	...
	def mocked_requests_get(url):
		...
		with open('jsonResponse.json') as data_file:    
		    repoMockJson = json.load(data_file)
		#pprint.pprint(repoMockJson)
		...
		if url == 'https://api.github.com/users/mmckeon16/repos':
			return MockResponse(repoMockJson, 200)
		elif url == 'https://api.github.com/repos/mmckeon16/Collect-Website/commits':
			return MockResponse(collectJson, 200)

		return MockResponse("Not valid input", 404)

	@mock.patch('requests.get', side_effect=mocked_requests_get)
	def testGetRepos(self, mock_get):		
		self.assertTrue(gitRequest.getRepos("mmckeon16"))
		#self.assertTrue(gitRequest.getRepos(123))

	@mock.patch('requests.get', side_effect=mocked_requests_get)
	def testGetCommits(self, mock_get):
		print("second commit test")
		self.assertEqual(gitRequest.getCommits("mmckeon16", "Collect-Website"), 72)
		self.assertTrue(gitRequest.getCommits(123, 5.968), "Not valid input")
		# self.assertTrue(gitRequest.getCommits("mmckeon16", "48"), 2)
		# self.assertTrue(gitRequest.getCommits("nfwjfjberfosermf", "fknzefj"), 2)
